...
def is_json_string(value):
    return (not value.isdigit()) and (not is_json_object(value)) and (not is_json_boolean(value))
...
def field_components_to_json(field_name, field_components, indent_level):
    ...
    for index, component in enumerate(field_components):
        ...
        if len(component_parts) == 1:
            ...
            component_parts_json = component_parts_to_json(component_name, component_parts, indent_level+1)

        json = json + build_json_property(component_name, component_parts_json, indent_level+1)
...
def segment_fields_to_json(segment_name, segment_fields, indent_level):
    ...
    for index, field in enumerate(segment_fields):
        ...
        if len(field_components) == 1:
            ...
            field_components_json = field_components_to_json(field_name, field_components, indent_level+1)

        json = json + build_json_property(field_name, field_components_json, indent_level+1)
...
def hl7_to_json(hl7_segments, indent_level=0):
    ...
    for index, segment in enumerate(hl7_segments):
        ...
        segment_fields_json = segment_fields_to_json(segment_name, segment_fields, indent_level+1)
        json = json + build_json_property(segment_name, segment_fields_json, indent_level+1)
